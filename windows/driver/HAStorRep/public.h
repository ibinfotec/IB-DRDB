...
} HA_GET_EVENTS_IN, *PHA_GET_EVENTS_IN;

typedef struct _HA_GET_EVENTS_OUT {
  uint32_t count;
  uint32_t reserved;
  // Followed by HA_EVENT[count]
} HA_GET_EVENTS_OUT, *PHA_GET_EVENTS_OUT;
...
typedef struct _HA_ACK_EVENTS_IN {
  ...
  uint32_t reserved;
} HA_ACK_EVENTS_IN, *PHA_ACK_EVENTS_IN;
...
// If still not defined (e.g. non-Windows or no headers), define manually or error?
// Assuming environment provides it if _WIN32.
#ifndef CTL_CODE
